    ...
    <header className="bg-green-700 text-white shadow-md">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex-grow flex justify-center">
          <nav className="flex space-x-6">
            ...
            <Link to="/gamification" className="hover:text-green-200 transition-colors">Challenges</Link>
      ...
      {isMenuOpen && (
        <nav className="md:hidden bg-green-800 px-4 py-2">
          ...
          <Link to="/gamification" className="block py-2 hover:text-green-200 transition-colors" onClick={() => setIsMenuOpen(false)}>Challenges</Link>
          <Link to="/emergency" className="block py-2 hover:text-green-200 transition-colors" onClick={() => setIsMenuOpen(false)}>Emergency</Link>
          <Link to="/settings" className="block py-2 hover:text-green-200 transition-colors" onClick={() => setIsMenuOpen(false)}>Settings</Link>
          ...
          <div className="border-t border-green-700 mt-2 pt-2">
            {isAuthenticated ? (
              <>
                ...
                <Link to="/profile" className="block py-2 hover:text-green-200 transition-colors" onClick={() => setIsMenuOpen(false)}>Your Profile</Link>
                <button
                  onClick={() => {
                    handleLogout();
                    setIsMenuOpen(false);
              ...
              <>
                <Link to="/login" className="block py-2 hover:text-green-200 transition-colors" onClick={() => setIsMenuOpen(false)}>Log in</Link>
                <Link to="/register" className="block py-2 hover:text-green-200 transition-colors" onClick={() => setIsMenuOpen(false)}>Sign up</Link>
              </>
            )}
          </div>
        </nav>
