...
class PollutionAgent:
    """
    Agent that uses Tavily to search for pollution data and processes it with Keywords AI
    """
    
    def __init__(self, tavily_service, keywords_ai_wrapper=None):
        """
        Initialize the pollution agent with Tavily service
        
        Args:
            tavily_service: Initialized Tavily service
            keywords_ai_wrapper: Optional Keywords AI wrapper for monitoring
        """
        self.tavily_service = tavily_service
        self.keywords_ai_wrapper = keywords_ai_wrapper
    ...
    async def _extract_pollution_data(self, search_results: Dict[str, Any], location: str) -> Dict[str, Any]:
        ...
        # Call Keywords AI API (using OpenAI-compatible endpoint)
        try:
            response = await self._call_keywords_ai(
                messages=[
            ...
            print(f"Error calling Keywords AI API: {str(e)}")
            return self._create_default_pollution_data(location)
    
    
    async def _call_keywords_ai(self, messages, model="gpt-4", temperature=0.1, max_tokens=1000):
        """
        Call Keywords AI API (OpenAI-compatible endpoint)
        ...
        Returns:
            API response
        """
        if not self.keywords_ai_api_key:
            raise ValueError("Keywords AI API key not found")
        
        headers = {
            "Authorization": f"Bearer {self.keywords_ai_api_key}",
